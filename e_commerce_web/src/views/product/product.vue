...
import {
  ...
  UpdateProductById, UpdateProductStatus
} from "@/api/product";
import {FindBrandByCategoryId} from "@/api/categoryBrand";
import {FindAllProductUnit} from "@/api/productUnit";
...
const queryDto = ref(queryForm);  //查询条件

const brandList = ref([]) //查询到的所有品牌列表
...
const saveData = async () => {
  const {code, message} = await SaveProduct(product.value)
  if (code === 200) {
...
const updateData = async () => {
  ...
  const {code, message} = await UpdateProductById(product.value)
  if (code === 200) {
    dialogVisible.value = false
    ElMessage.success(message)
    fetchData()
...
const remove = async id => {
  ...
  ElMessageBox.confirm('此操作将永久删除该记录, 是否继续?', 'Warning', {
    ...
    type: 'warning',
      ...
      .then(async () => {
        const {code, message} = await DeleteProductById(id)
        if (code === 200) {
          ElMessage.success(message)
          fetchData()
        }else {
...
const updateAuditStatus = async (id, auditStatus) => {
  const {code, message} = await UpdateProductAuditStatus(id, auditStatus)
  if (code === 200) {
...
const updateStatus = async (id, status) => {
  const {code, message} = await UpdateProductStatus(id, status)
  if (code === 200) {
    dialogAuditVisible.value = false
    ElMessage.success(message)
    fetchData()
